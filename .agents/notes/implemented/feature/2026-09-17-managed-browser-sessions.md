...
Login and mutations serialize durable writes before publishing cookies or revocations. Authenticated JSON mutations require an exact same-origin Origin, including scheme. The settings-general plugin adds a localized Login devices section only when the Host advertises managed mode. Connection's exact Fetch route owns the cookie response; generated RPC cannot set browser cookies. Lists and targeted operations are authority-scoped. Global logout and signing-key rotation intentionally revoke all authorities and also rotate the process launch token, returning one recovery URL. Current-cookie rotation replaces only the caller's session; another browser must log in again after revocation.

The device list keeps routine inspection separate from infrequent security maintenance. Each row shows a name, browser summary, recent activity, and current-device marker; its More menu owns rename, details, and the sole per-device logout action. Credential and link operations start behind Advanced security, with signing-key replacement inside a second Maintenance disclosure. Shared outline buttons make actions visible in both themes. Action-specific confirmation text preserves authority scope, current-device impact, and recovery-link requirements; disclosure is presentation only and changes no authorization.
...
Component tests cover collapsed defaults, browser summaries, rename cancellation and failure, selected-device targeting, action-specific confirmation cancellation, and recovery links. Browser expectations record the compact and expanded action lists; computed styles verify visible button borders, and real cookie-authenticated requests check that link replacement preserves sessions while rejecting the old link.
...
Showing every action and raw browser detail at once makes routine device inspection compete with global revocation. Removing those capabilities would lose incident-response tools. Collapsed advanced controls retain them with an extra navigation step instead of changing the security model.
